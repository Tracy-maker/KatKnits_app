...
export const useUpdateUser = () => {
  ...
  return useMutation({
    mutationFn: async (user: IUpdateUser) => {
      // Ensure that updateUser returns an object with a $id property
      const updatedUserData = await updateUser(user);
      return updatedUserData; // This should be an object with a $id property
    },
    onSuccess: (data) => {
      // Invalidate queries based on the returned data
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_CURRENT_USER],
      });
